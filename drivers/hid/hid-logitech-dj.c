...
static int logi_dj_ll_parse(struct hid_device *hid)
{
	...
	if (djdev->reports_supported & MULTIMEDIA) {
		dbg_hid("%s: sending a multimedia report descriptor: %x\n",
			__func__, djdev->reports_supported);
		/*
		 * Bluetooth receivers claim to have only 652 consumer usages,
		 * like most others, but some keyboards actually do use the
		 * 0x10xx logitech specific usage, like 27MHz devices.
		 */
		if (djdev->dj_receiver_dev->type == recvr_type_27mhz ||
		    djdev->dj_receiver_dev->type == recvr_type_bluetooth)
