 ...
 *
 * Note the uG3105 is not a full-featured autonomic fuel-gauge. Instead it is
...
#define UG3105_CURR_HYST_UA					60000
...
struct ug3105_chip {
	...
	struct mutex lock;
	u32 voltage_max_design;			/* micro-volt */
...
static int ug3105_get_status(struct ug3105_chip *chip)
{
	...
	if (chip->supplied && chip->ocv_avg > 4180000)
...
static int ug3105_get_capacity(struct ug3105_chip *chip)
{
	...
	for (i = 1; i < ARRAY_SIZE(ocv_capacity_tbl); i++) {
		...
		if (chip->voltage_max_design >= 4300000)
...
static void ug3105_work(struct work_struct *work)
{
	...
	/* chip->psy and chip->ocv_avg are protected by the lock */
	...
	chip->supplied = psy && power_supply_am_i_supplied(psy);
			       ...
			       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		regval = ug3105_read_word(chip->client, UG3105_REG_BAT_VOLT);
		if (regval < 0)
			return regval;
		val->intval = regval * chip->uv_per_unit;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX_DESIGN:
		val->intval = chip->voltage_max_design;
		break;
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		regval = ug3105_read_word(chip->client, UG3105_REG_BAT_CURR);
		if (regval < 0)
			return regval;
		val->intval = (s16)regval * chip->ua_per_unit;
		break;
...
static int ug3105_probe(struct i2c_client *client)
{
	...
	device_property_read_u32(dev, "voltage-max-design-microvolt",
				 &chip->voltage_max_design);
	device_property_read_u32(dev, "factory-internal-resistance-micro-ohms",
				 &intern_res_uohm);
	...
	chip->intern_res_avg = intern_res_uohm / 1000;
