...
int usbio_control_msg(struct usbio_client *client, u8 type, u8 cmd,
		      const void *obuf, u16 obuf_len, void *ibuf, u16 ibuf_len)
{
	return usbio_ctrl_msg(client->bridge, type, cmd, obuf, obuf_len, ibuf, ibuf_len);
}
...
int usbio_bulk_msg(struct usbio_client *client, u8 type, u8 cmd, bool last,
		   const void *obuf, u16 obuf_len, void *ibuf, u16 ibuf_len)
{
...
void usbio_lock(struct usbio_client *client)
{
	struct usbio_device *usbio = client->bridge;

	mutex_lock(&usbio->mutex);
}
EXPORT_SYMBOL_NS_GPL(usbio_lock, "USBIO");

void usbio_unlock(struct usbio_client *client)
{
	struct usbio_device *usbio = client->bridge;

	mutex_unlock(&usbio->mutex);
}
EXPORT_SYMBOL_NS_GPL(usbio_unlock, "USBIO");

void usbio_get_txrxbuf_len(struct usbio_client *client, u16 *txbuf_len, u16 *rxbuf_len)
{
	struct usbio_device *usbio = client->bridge;
...
void usbio_acpi_bind(struct usbio_client *client, const struct acpi_device_id *hids)
{
	struct device *dev = &client->adev.dev;
