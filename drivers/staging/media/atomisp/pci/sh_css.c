		     ...
		     struct ia_css_pipe_info *pipe_info)
{
	...
	if (!pipe || !pipe->stream) {
		pr_err(
				    "ia_css_pipe_get_info: ia_css_stream_create needs to be called before ia_css_[stream/pipe]_get_info pipe: %p\n", pipe);
		return -EINVAL;
	}
	/* we succeeded return the info */
	*pipe_info = pipe->info;
	pr_err("ia_css_pipe_get_info() leave\n");
	return 0;
}
		     ...
		     struct ia_css_stream **stream)
{
	...
	for (i = 0; i < num_pipes; i++) {
		...
		if (effective_res.height == 0 || effective_res.width == 0) {
			...
			pr_err("%s pipe %d setting effective res from input_config.effective_res\n", __func__, i);

			curr_pipe->config.input_effective_res = effective_res;
		}
		pr_err("%s pipe %d effective_res=%dx%d", __func__, i,
			   effective_res.width,
